...
1 ファイルごとに次の順で処理する。実行時にはステップごとの進捗は表示せず、
失敗したステップだけを `警告:` として表示する（Phase 1・3・4・5 の失敗は警告のみで、反転結果は出力される）。

| Phase | 内容 |
|-------|------|
| 1. レイアウト抽出 | music21 で読み込む前の元 XML から、direction・小節スタイル・縦方向レイアウトなどを抽出する。D.C. / D.S. の再生用ジャンプ指定もここで検出してレポートに記録する |
| 2. 反転と書き出し | music21 で小節順と小節内の音符を反転し、タイトルに「反転」を付与して書き出す。反転前後の音符数を比較し、一致しなければ警告する |
| 3. XML 後処理 | 出力 XML に対して順に実行する: direction 要素の復元 → スラー番号の正規化 → 臨時記号の再計算 → 複数小節休符・繰り返し記号の再配置 → 水平位置情報の除去 |
| 4. レイアウト適用 | Phase 1 で抽出したレイアウトを反転後の位置に変換して適用し、タイトルの credit を更新する |
| 5. 結合パートのレイアウト復元 | 複数譜のパートを music21 が結合した際に失われた print レイアウトを復元する |
...
問題があった場合は `警告:` / `エラー:` の行と、末尾に[問題レポート](#レポート例)が表示されます。
各ステップで何をしているかは[処理の流れ](#処理の流れ)を参照してください。
